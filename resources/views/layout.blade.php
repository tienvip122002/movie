...
<html lang="vi">
   ...
   <body class="home blog halimthemes halimmovies" data-masonry="">
                     ...
                     <ul id="menu-menu_1" class="nav navbar-nav navbar-left">
                        ...
                        <li class="mega dropdown">
                           ...
                           <ul role="menu" class=" dropdown-menu">
                              ...
                              <li><a title="{{ $value ->title }}" href="{{ route('genre', $value ->slug) }}">{{ $value ->title }}</a></li>
                        ...
                        <li class="mega dropdown">
                           ...
                           <ul role="menu" class=" dropdown-menu">
                              ...
                              <li><a title="{{ $value ->title }}" href="{{ route('country', $value ->slug) }}">{{ $value ->title }}</a></li>
                        ...
                        <li class="mega"><a title="{{ $value ->title }}" href="{{ route('category', $value ->slug) }}">{{ $value ->title }}</a></li>
      ...
      <div id="fb-root"></div>
<script async defer crossorigin="anonymous" src="https://connect.facebook.net/vi_VN/sdk.js#xfbml=1&version=v18.0" nonce="9rAecwMH"></script>
     ...
     $('document').ready(function() {
            ...
            $.getJSON("json_file/movies.json", function(data) {
               $.each(data, function (key, value) { 
                  if (value.title.search(expression) != -1) {
                     ...
                     $('#result').append('<li style="cursor:pointer; display: flex; max-height: 200px;" class="list-group-item link-class"><img src="uploads/movie/'+value.image+'" width="40" height="40" class="" /></br>'+value.title+'</li>')
                  }
               });
            })
         } else {
               ...
               var text_click = $(this).text().split('->');
               $('#timkiem').val($.trim(text_click[0]));
               $('#result').html('');
            });
     })
      ...
      //          if (search!=''){
      //             var expression = new RegExp(search,"i");
      //             $.getJSON("json_file/movies.json", 
      //                function (data) {
      //                   $.each(data, function (key, value) { 
      //                       if (value.title.search(expression) != -1) {
      //                         $('#result').css('display', 'inherit');
      //                         $('#result').append(<li style="cursor:pointer; display: flex; max-height: 200px;" class="list-group-item link-class"><img src="uploads/movie/'+value.image+'" width="100" class="" />
      //                            <div style="flex-direction: column; margin-left: 2px;"><h4 width="100%">'+value.title+'</h4>
      //                            <span style="display: -webkit-box; max-height: 8.2rem; -webkit-box-orient: vertical; overflow: hidden; text-overflow: ellipsis; white-space: normal; -webkit-line-clamp: 5; line-height: 1.6rem;" class="text-muted">| '+value.description+'</span></div></li>);
      //                       }
      //                   });
      //                });
      //          } else {
      ...
      $(document).ready(function() {
           //lấy ra topview ngày phim
             $.ajax({
                 url: "{{url('/filter-topview-df')}}",
                 method: "GET",
                 success:function(data) {
                     $('#show_data_df').html(data);
                 }
             });
             // click để chuyển tag sang tháng tuần
         $('.filter-sidebar').click(function() {
             ...
             $.ajax({
                 ...
                 success:function(data) {
                     $('#halim-ajax-popular-post-defaudt').css("display","none");
                     $('#show_data').html(data);
                 }
             });
         })
